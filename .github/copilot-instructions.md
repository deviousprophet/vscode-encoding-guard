...
- Users configure `encoding-guard.patternMap` (resource-scoped) to map patterns to a VS Code encoding ID (e.g. `"utf8"`, `"windows1252"`). Keys can be exact workspace-relative paths (e.g. `"data/report.csv"`), glob patterns (e.g. `"**/*.csv"`), or extension shorthand (e.g. `".csv"`). For any file that contains an `<?xml ... encoding="..."?>` declaration in its header, the extension automatically detects and applies that encoding when no `patternMap` match is found.
- Priority order for encoding resolution: exact path patterns → glob patterns (more specific first) → extension shorthand patterns → XML declaration → no action.
...
Key files
---------
- `src/detector.ts` — encoding detection: BOM checks, XML declaration parsing with byte-level XML preamble probing, heuristic UTF-8 round-trip fallback.
...
- `src/configManager.ts` — resolves the expected encoding for a URI from `patternMap` with smart priority (exact path → specific globs → extension shorthand); returns `null` when not mapped, which causes `applier.ts` to fall through to XML declaration detection.
...
- `src/test/applier.test.ts` — integration tests for `resolveTargetEncoding` across all file types and the `patternMap` config-wins-over-declaration rule.
...
Quick pointers
--------------
- `resolveTargetEncoding(uri, buf)` in `src/applier.ts` is the single coordination point: checks `patternMap` (via `configManager`), then falls back to `detectXmlDeclaration`.
...
- `patternMap` resolution priority is exact paths first, then more specific glob patterns, then extension shorthand defaults.
- Context menu commands *Set Extension Encoding...* and *Set File Encoding...* both write into `encoding-guard.patternMap`.
